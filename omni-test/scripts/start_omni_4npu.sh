...
# Qwen2.5-Omni-7B 4卡测试启动脚本
# 使用方法: bash start_omni_4npu.sh [max_model_len] [port] [npu_devices]

set -e

# 参数配置
MAX_MODEL_LEN=${1:-16384}
PORT=${2:-8001}
NPU_DEVICES=${3:-"4,5,6,7"}
MODEL_PATH="/models/Qwen/Qwen2___5-Omni-7B"
LOG_FILE="/home/bes/work/vllm-project/vllm_serve_omni_4npu.log"

echo "========================================"
echo "启动 Qwen2.5-Omni-7B 服务 (4卡)"
echo "========================================"
echo "NPU 设备: ${NPU_DEVICES}"
...
echo ""
echo "========================================"
echo "服务启动中，请等待约 5-6 分钟..."
echo "4卡部署会增加启动时间和编译时间"
